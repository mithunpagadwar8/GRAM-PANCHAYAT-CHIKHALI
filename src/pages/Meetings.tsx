...
const Meetings: React.FC<MeetingsProps> = ({ meetings }) => {
  const [filter, setFilter] = useState<
    "All" | "Gram Sabha" | "Masik Sabha" | "Bal Sabha" | "Ward Sabha"
  >("All");

  const filteredMeetings =
    filter === "All"
      ? meetings
      : meetings.filter((m) => m.type === filter);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 mt-2">
          Minutes, Schedules, and Recordings of Panchayat Meetings
        </p>
      </div>

      {/* Filter Tabs */}
      <div className="flex flex-wrap justify-center gap-2 mb-8">
        {["All", "Gram Sabha", "Masik Sabha", "Ward Sabha", "Bal Sabha"].map(
          (type) => (
            <button
              key={type}
              ...
              className={`px-6 py-2 rounded-full font-bold text-sm transition-colors ${
                filter === type
                  ? "bg-gov-primary text-white shadow-lg"
              ...
              <div className="p-6 md:w-2/3 flex flex-col justify-between">
                ...
                <div>
                  <button className="text-blue-600 hover:text-blue-800 font-semibold text-sm">
                    ...
                    View Minutes / Resolution (Tharav)
                  </button>
                </div>
              </div>
            </div>
